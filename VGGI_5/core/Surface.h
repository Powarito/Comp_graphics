...
public:
    ...
    // Generate mesh vertices with normals
    // Returns: position.xyz, normal.xyz (6 floats per vertex)
    std::vector<float> generateMeshVertices() const {
        std::vector<float> vertices;
        vertices.reserve((uSegments + 1) * (vSegments + 1) * 6);
    ...
    // Generate normal visualization lines for debugging
    // Returns: pos(3) + color(3) for each line segment (start and end points)
    std::vector<float> generateNormalVisualization(float normalLength = 0.3f) const {
        std::vector<float> vertices;

        float uStep = (uMax - uMin) / uSegments;
        float vStep = (vMax - vMin) / vSegments;

        // Sample normals at each vertex
        for (int i = 0; i <= vSegments; ++i) {
            float v = vMin + i * vStep;
            ...
            for (int j = 0; j <= uSegments; ++j) {
                ...
                glm::vec3 pos = evaluate(u, v);
                glm::vec3 normal = computeNormal(u, v);
                glm::vec3 endPos = pos + normal * normalLength;

                // Line start (surface point) - cyan color
                vertices.insert(vertices.end(), {
                    pos.x, pos.y, pos.z,
                    0.0f, 1.0f, 1.0f
                    });

                // Line end (normal tip) - magenta color
                vertices.insert(vertices.end(), {
...
private:
    ...
    inline glm::vec3 computeNormal(float u, float v) const {
        ...
        glm::vec3 pMinus, pPlus;

        // Partial derivative with respect to u (dP/du)
        pMinus = evaluate(u - eps, v);
        pPlus = evaluate(u + eps, v);
        glm::vec3 dPdu = (pPlus - pMinus) / (2.0f * eps);

        // Partial derivative with respect to v (dP/dv)
        pMinus = evaluate(u, v - eps);
        pPlus = evaluate(u, v + eps);
        glm::vec3 dPdv = (pPlus - pMinus) / (2.0f * eps);

        // Normal is cross product: dPdu x dPdv
        // Order matters for direction (outward vs inward)
        ...
        if (len > 1e-6f) {
            ...
            // For closed surfaces like pear/sphere, we want normals pointing outward
            // Check if normal points away from center (approximate center at origin)
            glm::vec3 pos = evaluate(u, v);
            glm::vec3 toCenter = -pos; // Vector from point to origin
        ...
        // Fallback for degenerate cases (poles)
        // Use position vector as approximation for outward normal
        glm::vec3 pos = evaluate(u, v);
        len = glm::length(pos);
        if (len > 1e-6f) {
            return pos / len;
